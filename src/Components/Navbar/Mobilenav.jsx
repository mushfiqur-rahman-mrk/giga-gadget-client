...
const Mobilenav = () => {
  ...
  <li className="font-semibold text-md dark:text-white"><NavLink to="/my-cart" className={({ isActive, isPending }) =>isPending ? "pending" : isActive ? "text-red-500 underline" : ""}>My cart</NavLink></li>
    ...
    return (
        ...
        <>
            ...
            <div className=' w-full text-center'>
              <ul className={`bg-gradient-to-r from-cyan-500 to-blue-500 z-30 space-y-4 pt-5 border absolute w-full h-full ${open ? 'top-18 transition delay-1000' : 'hidden'}`}>
                 {navLinks} 
                 
                 {
              user ? <div onClick={handleLogout}>
              <button className="px-2  w-full bg-cyan-800 py-3 text-white rounded-lg">Log out</button>
            </div>
            :
            <Link to={'/signup'}>
              <button className='w-full bg-cyan-800 py-3 text-white'>Login</button>
            </Link>
